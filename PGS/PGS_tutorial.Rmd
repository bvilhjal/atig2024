...
```{r}
# the function which finds the SNP-overlap needs some specific columns to run, therefore, we make sure we have everything we need.

# sumstats$n_eff <- 4 / (1 / sumstats$n_case + 1 / sumstats$n_control) # The standard formula to derive effective sample size, if not provided
...
First, you need to compute correlations between variants. Florian recommends using a window size of 3 cM (see the [**LDpred2 paper**](https://doi.org/10.1093/bioinformatics/btaa1029)).
