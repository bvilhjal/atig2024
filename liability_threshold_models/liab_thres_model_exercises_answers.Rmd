...
These exercises, we will investigate concepts from the liability threshold model and time-to-event (survival) analysis.
...
We will:
 ...
 - Explore how different biases affect the data
...
The liability threshold model assumes that each individual has an underlying liability or genetic predisposition, which follows a normal distribution. Only individuals whose liability exceeds a certain threshold are considered cases.
...
In this exercise, we will add an environmental component to the liability model to reflect the influence of non-genetic factors. This allows us to model a more realistic scenario where liability is influenced by both genetic and environmental factors.
...
##### A4: As random noise, independent of the genetic factors, and its variance is determined by the proportion of variance not explained by genetics (i.e., 1 − heritability).
...
DELETE:
This randomness reflects real-world scenarios where individuals with similar genetic risk can have different outcomes due to environmental influences. Consequently, some individuals who would not have been classified as cases based solely on their genetic liability may now be classified as cases due to the added environmental component.
...
DELETE
NB:In reality, complex traits are often influenced by thousands of genetic variants with small effects. When using a limited number of SNPs, the genetic variance might not be fully captured.  
...
liability_ascertained <- liability_env[ascertained_sample]
...
heritability_est
```
##### Q7: How does ascertainment bias alter the distribution of liability in the sample compared to the full population?
##### Q8: Why do effect size estimates and heritability estimates become biased in ascertained samples?
...
We simulate both the time to event (e.g., time to disease onset) and censoring times (e.g., time when an individual's data is no longer available). The Cox model will help us analyze how genetic factors affect the risk of experiencing the event.

```{r}

# Simulate survival times
set.seed(123)
time_to_event <- rexp(n_individuals, rate = exp(liability_env - mean(liability_env)))
...
plot(km_fit, col = c("blue", "red"), lty = 1:2, xlab = "Time", ylab = "Survival Probability", xlim = c(0, 15), main = "Kaplan-Meier Curve by Phenotype")
...
##### Q11: Can you think of examples from genomics where censoring might occur?

##### Q12: How might you design a real-world study to avoid biases like ascertainment or censoring? What factors would you need to consider in a genomic study with survival outcomes?
